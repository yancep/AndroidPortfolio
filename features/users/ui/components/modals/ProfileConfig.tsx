...
import PersonalInformationSection from "@/features/users/ui/components/PersonalInformationSection";
import { SecurityConfigSection } from "@/features/users/ui/components/SecurityConfigSection";
...
export function ProfileConfig({
  ...
  function Item(props: { option: ItemOption; index: number; action: () => void }) {
    return (
      <div
        className={
          "hover:bg-default rounded-md p-2 flex flex-row items-center justify-between cursor-pointer"
        }
  ...
  return (
    <div className={"flex flex-row h-full  w-full "}>
      <div
        className={"flex space-y-4 flex-col h-full p-4 w-[270] items-start justify-between"}
      >
        <>
          <User
            as="button"
      ...
      <div
        className={
          "items-center  flex-1 justify-center  border-l-2 border-l-default flex w-full h-full overflow-y-auto p-4"
        }
      >
        <RenderSection />
      </div>
      <LogoutModal isOpen={isOpen3} onClose={onClose3} />
    </div>
  );
}
